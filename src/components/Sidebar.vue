...
<template>
      ...
      <div id="my-scroll" style="margin: 6px 14px 0 14px">
        <ul class="nav-list" style="overflow: visible">
          ...
          <li v-for="(menuItem, index) in menuItems" :key="index" :id="'links_' + index">
            <router-link v-if="isUsedVueRouter && showMenuItem(menuItem)" :to="menuItem.link">
              <i class="bx" :class="menuItem.icon || 'bx-square-rounded'" />
              <span class="links_name">{{ menuItem.name }}</span>
            </router-link>
            <a v-else-if="!isUsedVueRouter && showMenuItem(menuItem)" @click.stop.prevent="$emit('menuItemClicked', menuItem.link)" :href="menuItem.link">
              <i class="bx" :class="menuItem.icon || 'bx-square-rounded'" />
              <span class="links_name">{{ menuItem.name }}</span>
            </a>
  ...
  <div class="grouped-link">
    <a href="#" @click="toggleGroupedItems">
      <i class="bx bx-group" />
      <span class="links_name">Jobs➭</span>
    </a>
    <ul v-show="showGroupedItems">
      <li v-for="(groupedItem, index) in jobs" :key="index">
        <a :href="groupedItem.link">
          <i class="bx" :class="groupedItem.icon || 'bx-square-rounded'" />
          <span class="links_name">{{ groupedItem.name }}</span>
        </a>
      </li>
    </ul>
  </div>
...
export default {
  ...
  props: {
    ...
    //   // Add more grouped items as needed
    // ]
    //     },
        {
          link: '/oldApp',
  ...
  methods: {
    ...
    // Check if the menu item should be shown based on the current user's roles
    if (menuItem.role === 'ROLE_ADMIN') {
      return this.currentUser.roles.includes('ROLE_ADMIN');
    } else if (menuItem.role === 'ROLE_USER') {
      return this.currentUser.roles.includes('ROLE_USER');
    } else if (menuItem.role === 'ROLE_MODERATOR') {
      return this.currentUser.roles.includes('ROLE_MODERATOR');
    }
    // Return true for menu items without a specified role
    return true;
  },
  toggleGroupedItems() {
    this.showGroupedItems = !this.showGroupedItems;
  },
    tooltipAttached() {
